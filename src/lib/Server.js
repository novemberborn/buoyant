...
export default class Server {
  ...
  async join (addresses = []) {
    ...
    try {
      const nonPeerStream = await this._transport.listen()
      await this._raft.joinInitialCluster({
        addresses,
        connect: async opts => this._transport.connect(opts),
        nonPeerStream
      })
    } catch (err) {
      if (this._closeInProgress) {
